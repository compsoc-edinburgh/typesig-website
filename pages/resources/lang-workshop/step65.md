...
### Reduction Rules
To express how programs evaluate, we'll formally introduce the $\rightsquigarrow$ judgement, read as "steps to". This relation roughly corresponds to one run of your `eval` function, and is very similar to the `~>` notation we've used before.
...
The $\text{dom}$ function just returns a set of the variables of either environments $\rho$ or typing contexts $\Gamma$.
