...
The problem is, there's no way to figure out if the expression above the line is well-typed. What type is $\texttt{x}$? Is it an $\texttt{Int}$, like we want, or could it be a $\texttt{Bool}$?
